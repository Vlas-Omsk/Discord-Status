    ...
    class ProcessExtension
    {
        ...
        public static string[] GetCommandLine(int id)
        {
            if (!string.IsNullOrEmpty(Process.GetProcessById(id).StartInfo.Arguments))
                return GetCommandLineParams(Process.GetProcessById(id).StartInfo.Arguments);
            else
            {
                var m = GetConsoleCommandOut("util\\GetProcessCommandLine32", id.ToString());
                if (m.StartsWith("Err"))
                {
                    var err = Math.Abs(Convert.ToInt32(m.Substring(4, 2))) - 10;
                    throw new Exception(new Json(File.ReadAllText("util\\GetProcessCommandLineErrors.json"))[0][err].Value);
                }
                else
                    return GetCommandLineParams(m);
            }
        }

        public static string GetConsoleCommandOut(string path, string args)
        {
        ...
        private static int GetParentProcessId(int id)
        {
            try
            {
                return int.Parse(GetConsoleCommandOut("WMIC", "process where processid=" + id + " get parentprocessid").Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries)[1]);
            }
            catch { return -1; }
        }

        public static string[] GetCommandLineParams(string str)
        {
        ...
        /*[DllImport("CPPmethods\\ProcCmdLine32.dll", CharSet = CharSet.Unicode, EntryPoint = "GetProcCmdLine")]
        public extern static bool GetProcCmdLine32(uint nProcId, StringBuilder sb, uint dwSizeBuf);

        [DllImport("CPPmethods\\ProcCmdLine64.dll", CharSet = CharSet.Unicode, EntryPoint = "GetProcCmdLine")]
        ...
        public static string GetCommandLine(int id)
        {
            ...
            switch (IntPtr.Size)
            {
                case 4: GetProcCmdLine32((uint)id, sb, (uint)sb.Capacity); break;
                case 8: GetProcCmdLine64((uint)id, sb, (uint)sb.Capacity); break;
            }
            return sb.ToString();
        }

        public static string OGetCommandLine32(int id)
        {
            return GetProcessOut(@"G:\CSharp\WarfaceStatus\GetProcess\Release\GetProcess.exe", id.ToString());
        }

        public static string OGetCommandLine64(int id)
        {
            return GetProcessOut(@"G:\CSharp\WarfaceStatus\GetProcess\x64\Release\GetProcess.exe", id.ToString());
        }*/
        /*
        private static string GetCommadasfasfdLine(Process process)
        {
            ...
            IntPtr outPtr = Marshal.PtrToStructure<PROCESS_BASIC_INFORMATION>(pbi).PebBaseAddress;

            var peb_size = Marshal.SizeOf(typeof(PEB));
            byte[] buffer = new byte[peb_size];
            IntPtr bytesread;
            var n = ReadProcessMemory(hProc, outPtr, buffer, peb_size, out bytesread);
